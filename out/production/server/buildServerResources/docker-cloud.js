...
BS.Clouds.Docker = BS.Clouds.Docker || (function () {
        ...
        var self = {
            ...
            _checkConnection: function () {
                ...
                if (self._checkConnectionInProgress()) {
                    return false;
                }

                self.checkConnectionDeferred = $j.Deferred()
                ...
                BS.ajaxRequest(self.checkConnectionUrl, {
                    ...
                    onSuccess: function (response) {
                        ...
                        if ($errors.length) {
                            self.checkConnectionDeferred.reject($errors.text());
                        } else {
                            self.checkConnectionDeferred.resolve(response);
                        }
                    }
                });
            ...
            _checkConnectionInProgress: function () {
                return self.checkConnectionDeferred ?
                self.checkConnectionDeferred.state() === 'pending' :
                    false;
            },
            ...
            _bindHandlers: function () {
                ...
                /* Test container */
                self.$imageTestContainerStartBtn.click(function() {

                    var viewModel = self._restoreViewModel();
                    var settings = self._convertViewModelToSettings(viewModel);
                    self.$testedImage.val(JSON.stringify(settings));

                    BS.ajaxRequest(self.testContainerUrl + '?action=submit', {
                        parameters: BS.Clouds.Admin.CreateProfileForm.serializeParameters(),
                        onSuccess: function(response) {
                            console.log('Test request submitted.');
                            var $response = $j(response.responseXML);
                            var taskUuid = $response.find("taskUuid").text();
                            console.log('uuid: ' + taskUuid);
                            self._pollingImageTestStatus(taskUuid);
                            return response;
                        }
                    });
                });
            ...
            _pollingImageTestStatus: function(uuid) {
                console.log('setting pooling.');
                    BS.ajaxRequest(self.testContainerUrl + '?action=query&uuid=' + uuid, {
                        onSuccess: function (response) {
                            console.log('Querying status');
                            var $response = $j(response);
                            var $msgs = $response.find("msg");
                            self.$dockerTestContainerOutput.val();
                            var output = "";
                            $j.each($msgs, function(i, msg) {
                                output += msg + "\n";
                            });
                            console.log('updating output.');
                            self.$dockerTestContainerOutput.val(output);
                            if ($response.find('status').text() != 'PENDING') {
                                console.log("the response: " + $response.text());
                                console.log('Test done: ' + $response.children('status').text());
                            } else {
                                setTimeout(self._pollingImageTestStatus, 10000);
                            }
                        }
                    });
            },
